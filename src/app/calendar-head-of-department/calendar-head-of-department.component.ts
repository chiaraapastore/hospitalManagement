...
@Component({
  selector: 'app-calendar-head-of-department',
  templateUrl: './calendar-head-of-department.component.html',
  styleUrls: ['./calendar-head-of-department.component.css']
})
export class CalendarHeadOfDepartmentComponent  implements OnInit, OnDestroy {


  constructor(public dialog: MatDialog, private location: Location) {}
  private totalFerie = 30;
  private ferieEstive = 20;
  events: EventInput[] = [];
  forceRerender = false;
  ngOnInit() {
    this.updateEvents();
  ...
  private generateFerie(anno: number): EventInput[] {
    ...
    const startDate = new Date(anno, 6, 1);
    ...
    while (ferieDistribuite < remainingFerie) {
      ...
      if (ferieDay.getMonth() >= 6 && ferieDay.getMonth() <= 8) continue;
